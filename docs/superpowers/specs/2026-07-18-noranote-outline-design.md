...
3. 7/18–7/24 七日冲刺表，保持日期、阶段、当日主线、快捷 MD 笔记和推荐教程/资料五列。
4. 九个专栏的四列表格地图。
...
README 的日期导航保留现有五列结构，不为追求字段完整而把首页表格做得更宽：

| 日期 | 阶段 | 当日主线 | 快捷 MD 笔记 | 推荐教程/资料 |
| --- | --- | --- | --- | --- |

- “当日主线”继续用一句话汇总当天的多个任务。
- “快捷 MD 笔记”链接到为本人记笔记准备的空壳文章；这些空壳是当前阶段的必要产物，应当保留。
- “推荐教程/资料”不能只写“教程”或只给一条泛链接。每个日期显示 2–4 个最重要的直达资源，并提供一个“当日完整资源”链接，跳到资源页的对应日期锚点。
- 预计用时、完成产物、状态和返工标记放在 `daily-plan.md` 的完整任务表中，不挤进 README 的五列表格。
...
`daily-plan.md` 保留“任务行”粒度；README 保留“日期汇总行”粒度。两者分别服务于执行和快速导航，不合并成一张过宽的总表。
...
- 建议阅读或观看范围。
- 预计耗时。
- 讲解特征：配图/演示程度，或生动易懂、口碑与热度依据。
- 看完能够回答的问题。
- 是否为一手资料。
- 链接核验日期。

资源覆盖采用“每日入口 + 文章入口”两级结构：

- `08-resources/02-july-18-to-24-must-use.md` 按日期汇总当天所有任务的资源，是 README 每日行的完整资源入口。
- Android、项目/后端/支付、Agent 三条学习路径页面按文章长期归档资源，是每篇空壳文章后续补正文时的资源入口。
- 同一资源只维护一份详细说明，其他位置使用链接引用，避免多个清单逐渐不一致。

资源数量以“覆盖任务且当天看得完”为标准，不追求把全网链接全部塞入表格：

- 每个独立 P0 任务至少有 1 个必看资源。
- Activity、Handler、Compose/MVVM、网络请求链路、项目架构、支付闭环、Agent 工具调用等复杂主题，优先提供图解或演示资源；没有合适的图解时，可使用讲解生动、广受好评、热度高且适合速通的教程替代，并另外保留官方或一手资料用于事实核验。
- 每个任务最多标记 1–2 个“必看”，其余放入“补充/7 月 24 日后”，避免链接越全越无法执行。
- 如果现有完整资源清单不能覆盖当天全部任务，就新增资源；不能用一个“教程”链接代替多个不同主题。

筛选顺序：

1. 有明确时长、步骤和可见产出的官方课程或 codelab。
2. 有架构图、时序图、代码演示或项目成品的一手资料。
3. 即使没有配图或演示，只要讲解生动易懂、口碑稳定、热度高且能在有限时间内形成面试回答，也可以作为优先资源。
4. 能补充中文解释的高质量二手资料。
5. 长篇参考文档只放在 7 月 24 日后资源区。
...
- README 的每日导航保持“日期、阶段、当日主线、快捷 MD 笔记、推荐教程/资料”五列结构。
- 每个 README 日期行有 2–4 个必看直达链接和一个当日完整资源入口，且完整入口覆盖该日所有学习任务。
- 所有 Markdown 内链有效。
- 不存在孤立文章、重复主题或旧路径残留。
- README、专栏 README 和 `daily-plan.md` 的名称与链接一致。
- 外部推荐链接能够打开，并符合速通要求；图解或演示是优先项，但生动易懂、口碑稳定且热度高的资源可以替代。
